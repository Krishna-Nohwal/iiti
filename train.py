...
def cls_loss(logits, features, labels, lam):
    """CE + SupCon on any (logits, features, labels) triple."""
    return bce_loss(logits, labels) + lam * supcon_loss(features, labels)

def video_loss(video_logits, video_feats, frame_logits_list, labels, lam):
    """
    ...
    Args:
        ...
        lam               : supcon weight
    """
    l_primary = cls_loss(video_logits, video_feats, labels, lam)
    ...
    l_aux = sum(
        bce_loss(fl, frame_labels) for fl in frame_logits_list
    ) / len(frame_logits_list)

    return l_primary + 0.3 * l_aux
...
def run_eval(model, loader, desc, device):
    ...
    with torch.inference_mode(), torch.autocast(device_type=device.type, dtype=torch.float16):
        for frames, labels in tqdm(loader, desc=desc, leave=False):
            ...
            video_logits, _, _, _ = model(frames)
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (frames, labels) in enumerate(
            ...
            with torch.autocast(device_type=device.type, dtype=torch.float16):
                video_logits, frame_logits_list, _, video_feats_list = model(frames)
                loss = video_loss(
                    video_logits,
                    video_feats_list[3],   # deepest layer's video feature for SupCon
                    frame_logits_list,
                    labels,
                    lam,
                )

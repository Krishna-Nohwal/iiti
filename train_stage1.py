"""
train_stage1.py — Stage 1 training script for DINO-MAC.

This is train.py verbatim, with two changes only:
  1. `from frame_model import ViT` instead of `from model import ViT`
  2. model() now returns (logits_list, features_list, cls_list); the extra
     cls_list is unpacked and discarded so all other logic is unchanged.
...
from frame_model_small import ViT   # <-- only change from train.py

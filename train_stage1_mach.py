"""
train_stage1.py — Stage 1 training script for DINO-MAC (JointMACHead variant).

Model returns (logits_list, features_list, cls_list) for video_model
compatibility, where:
    logits_list[0..2] = aux logits  (cls_aux, reg_aux, avg_aux)
    logits_list[3]    = primary fused logits
    features_list     = mirrors logits_list
    cls_list          = raw per-layer CLS tokens (unused in stage 1)

Loss structure:
    l_primary = CE(logits_list[3]) + lam * SupCon(features_list[3])
    l_aux     = mean of [CE(logits_list[i]) + lam * SupCon(features_list[i])
                         for i in {0, 1, 2}] * aux_weight
...
from frame_model_mach import ViT
...
parser.add_argument('--aux_weight',    default=0.25, type=float,
                    help='Loss weight for the 3 per-projector auxiliary heads.')
...
def total_loss(logits_list, features_list, labels, lam, aux_weight):
    """
    logits_list[3] / features_list[3] : primary fused output  -> full weight
    logits_list[0..2] / features_list[0..2] : aux projector outputs -> aux_weight / 3
    """
    l_primary = cls_loss(logits_list[3], features_list[3], labels, lam)
    l_aux = aux_weight * sum(
        cls_loss(logits_list[i], features_list[i], labels, lam)
        for i in range(3)
    ) / 3.0
...
def run_eval(model, loader, desc, device):
    ...
    with torch.inference_mode(), torch.autocast(device_type=device.type, dtype=torch.float16):
        for imgs, labels in tqdm(loader, desc=desc, leave=False):
            ...
            # logits_list[3] is the primary fused logit, consistent with stage2
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (imgs, labels) in enumerate(
            ...
            with torch.autocast(device_type=device.type, dtype=torch.float16):
                ...
                loss = total_loss(logits_list, features_list, labels, lam, aux_weight)

...
def _extract_video_id(sample_dir: str) -> str:
    ...
    Strategy: strip the frame suffix by finding the last occurrence of
    '_frame_' (training) or '_f' followed by digits (CDF).
    """
    import re
    ...
    # Try training format first: _frame_<digits>
    idx = basename.rfind("_frame_")
    if idx != -1:
        clip_id = basename[:idx]
    else:
        # CDF format: _f<digits> at end of string
...
def build_memory_bank(
    ...
    Runs the frozen model in inference mode over every real video in the
    training dataset, collects the temporal transformer output embeddings,
    and stores them in the bank.  Only real videos (label == 0) are stored.

    The bank is built once before training starts and never updated — valid
    because the backbone is frozen so embeddings are stable throughout.

    Args
    ----
    model         : VideoViT  (frame_model must already be frozen)
    train_dataset : ManifestVideoDataset
    k             : number of nearest neighbours to retrieve at query time
    num_workers   : DataLoader workers

    Returns
    -------
    bank : RealVideoMemoryBank  (built and ready for attach_memory_bank)
    """
    print("
  Building real-video memory bank …")

    # Separate loader over real videos only — no balancing needed.
    ...
    bank = RealVideoMemoryBank(
        ...
        num_heads = VideoViT.NUM_HEADS,
        k         = k,
    )

    # Unwrap torch.compile if active so we can access submodules directly.
